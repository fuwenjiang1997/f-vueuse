...
import { useWidthLoading } from '../index'

describe('useWidthLoading', () => {
  it('should initialize with isLoading false', () => {
    const [, isLoading] = useWidthLoading(done => {
      done()
    })
  ...
  it('should set isLoading to true when handler is called', async () => {
    const [handler, isLoading] = useWidthLoading(done => {
      setTimeout(done, 100)
    })
  ...
  it('should call fn with done callback and args', async () => {
    ...
    const [handler, isLoading] = useWidthLoading(fn)
  ...
  it('should width result', async () => {
    const [handler, isLoading] = useWidthLoading((done, param) => {
      setTimeout(() => {
        done(`hello ${param}`)
      }, 10)
    })
